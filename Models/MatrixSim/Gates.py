    ...
    def gate_matrix(self):
        if self._gate_matrix is None:
            if self.N == 1:
                return self.gate
            gate_list = [
                np.identity(2) if i != self.target else self.gate for i in range(self.N)
            ]
...
class MultiQubitGate:
    ...
    def gate_matrix(self):
        if self._gate_matrix is None:
            first_term = [
                np.identity(2) if i != self.control else HELPERS["|0><0|"]
                for i in range(self.N)
            ]
            first_term = reduce(lambda x, y: np.kron(x, y), first_term)

            second_term = [
                np.identity(2) if i != self.control else HELPERS["|1><1|"]
                for i in range(self.N)
            ]
